...
float sdf(vec3 current) {
    ...
    return subtract(boxSDF(current, vec3(1, 1, 1)), sphereSDF(vec3(0, 0, 0), 1.3, current));
}
...
void main() {    
    ...
    vec3 step_direction = normalize(world_pos.xyz - camera_pos);
    ...
    for (int i = 0; i < max_steps; i++) {
        ...
        if (dist <= eps) {
            ...
            vec3 normal = calculateNormal(ray_base);

            out_color = vec4(ambientFactor * sphere_color, 1);
            out_color += vec4(diffuseFactor * max(dot(light_dir, normal), 0) * sphere_color, 1);
            //out_color = vec4(normal, 1);
            break;
        }
        ray_base += step_direction * dist;
    }
}
