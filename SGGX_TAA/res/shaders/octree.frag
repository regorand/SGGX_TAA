...
tree_traversal_leaf_s getLeafAtPosition(vec3 position, uint max_depth) {
...
tree_traversal_leaf_s getLeafAtPosition2(vec3 position, 
        uint max_depth, 
        ...
        inout Stack inner_index_stack)
{
    while (!isInBox(node_AABB, position)) {
        if (!hasElement(parent_stack) || !hasElement(inner_index_stack)) {
            return false;
        }
...
void main() {
    ...
    if (output_type == 6) {
        ...
        tree_traversal_leaf_s leaf_node  = getLeafAtPosition2(position, 
                max_depth, 
                current_depth, 
                0,
                octreeBound, 
        ...
        tree_traversal_leaf_s leaf_node  = getLeafAtPosition(position, max_depth);
        ...
        for (int i = 0; i < max_steps; i++) {
            ...
            if (last_node_index != leaf_node.node_index) {
                out_color += (leaf.value / 30) * vec4(1);
                last_node_index = leaf_node.node_index;
            }
        }
    } else if (output_type == 2) {

        vec3 halfway = (higher + lower) / 2;


        float t = outline_intersection.t_far - outline_intersection.t_near / 2;
        vec3 position = ray_base + t * ray_dir;
        ...
        AABB aabb = AABB(leaf_node.lower, leaf_node.higher);

        for (int i = 0; i < current_depth; i++) {
            get_parent_AABB(aabb, pop(inner_index_stack));   
        }
	    ...
	    vec3 one_minus_factors = 1 - factors;
        vec3 current_higher = factors * higher + one_minus_factors * halfway;
	    vec3 current_lower = one_minus_factors * lower + factors * halfway;

        AABB aabb = AABB(current_lower, current_higher);
        */
        vec3 diff_lower = (lower) - aabb.lower;
        vec3 diff_upper = higher - aabb.upper;
        
        out_color = vec4(abs(diff_lower), 1);
        ...
        while (isInBox(octreeBound, position)) {
            ...
            position = ray_base + leaf_intersection.far_dimension * ray_dir + ray_eps;

            uint node_index = 0;

            bool result = backUpToNodeContaining(position, node_index, leaf_AABB, parent_stack, inner_index_stack);

            vec3 diff_lower = lower - leaf_AABB.lower;
            vec3 diff_upper = higher - leaf_AABB.upper;

            if (!result) {
                out_color = vec4(abs(diff_lower), 1);
                
                // Went out of Octree AABB, no more rendering needed
                break;
            }
            ...
            tree_traversal_leaf_s leaf_node = getLeafAtPosition2(position, 
                    ...
                    parent_stack, 
                    inner_index_stack);
            node = nodes_data[leaf_node.node_index];
            leaf_intersection = getBoxIntersection(pixel_ray, leaf_AABB);
